...
class ModuleStoreLocatorImporter {
    ...
    public function showImport(): string {
        ...
        if( Input::post('FORM_SUBMIT') == 'tl_storelocator_import' ) {
            ...
            $arrUploaded = $uploader->uploadTo('system/tmp');
            ...
            if( !empty($arrFiles) ) {
                ...
                $autoIncrement = Database::getInstance()->prepare("
                ...
                foreach( $arrFiles as $file ) {
                    ...
                    while( ($data = fgetcsv($objFile->handle)) !== FALSE ) {
                        ...
                        try {
                            Database::getInstance()->prepare("INSERT INTO `tl_storelocator_stores` (`pid`,`tstamp`,`name`,`alias`,`email`,`url`,`phone`,`fax`,`street`,`postal`,`city`,`country`,`longitude`,`latitude`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)")->execute(
                                $pid
                            ,   time()
                            ,   $data[0]
                            ,   $alias
                    ...
                    Controller::redirect(str_replace('&key=importStores', '', Environment::get('request')));
                }
            }
        }
        ...
        return '
            <div id="tl_buttons">
                <a href="'.StringUtil::ampersand(str_replace('&key=importStores', '', Environment::get('request'))).'" class="header_back" title="'.StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']).'" accesskey="b">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
            </div>
            '.Message::generate().'
            <form action="'.StringUtil::ampersand(Environment::get('request'), true).'" id="tl_storelocator_import" class="tl_form tl_edit_form" method="post" enctype="multipart/form-data" onsubmit="AjaxRequest.displayBox(\''.$GLOBALS['TL_LANG']['tl_storelocator']['import']['ajax_import_running'].'\');">
                <div class="tl_formbody_edit sl_import">
                    ...
                    <div class="tl_tbox widget">
                        <h3 style="margin-bottom: 10px;">'.$GLOBALS['TL_LANG']['tl_storelocator']['import']['head'].'</h3>'.$uploader->generateMarkup().(isset($GLOBALS['TL_LANG']['tl_storelocator']['import']['file'][1]) ? '
                ...
                <div class="tl_formbody_submit">
                    <div class="tl_submit_container">
                        <input type="submit" name="save" id="save" class="tl_submit" accesskey="s" value="'.StringUtil::specialchars($GLOBALS['TL_LANG']['tl_storelocator']['import']['start']).'">
                    </div>
                </div>
            </form>';
    }
